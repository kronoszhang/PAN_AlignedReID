...
def compute_dist(array1, array2, type='euclidean'):
  ...
  if type == 'cosine':
    ...
    return dist
  else:  # `euclidean` 's matrix computation method
    # shape [m1, 1]
...
def meta_local_dist(x, y):
  ...
  dist_mat = (np.exp(eu_dist) - 1.) / (np.exp(eu_dist) + 1.)  # for scale to [0,1]
...
def parallel_local_dist(x, y):
  """Parallel version.  equal to `serial_local_dist` , but more fast
  Args:
    x: numpy array, with shape [M, m, d]
    y: numpy array, with shape [N, n, d]
  Returns:
...
def local_dist(x, y):
  # from paper or `Model.py`, we know : `global_feat` with shape [N, C] and `local_feat` with shape [N, H, c]
  # and `local dist` used for `local_feat`
  # eg: x = [[2,3,4],[1,2,5]]  ====>  x.ndim = 2
  if (x.ndim == 2) and (y.ndim == 2):  # if `x` and `y` are matrix with shape : num1 * num2  ==> means N = 1
    return meta_local_dist(x, y)       # direct compute without parallel method
  elif (x.ndim == 3) and (y.ndim == 3): # if `x` and `y` are matrix with shape : num1 * num2 * num3 ==> means N != 1
    return parallel_local_dist(x, y)   # compute with parallel method 
  else:
...
def low_memory_matrix_op(
  ...
  Args:
    func: a matrix function func(x, y) -> z with shape [M, N]  ===> i.e. matrix operation
