...
def set_devices_for_ml(sys_device_ids):
  ...
  for ids in sys_device_ids:
    ...
    """
    eg:
    sys_device_ids = ((0,1,), (0,), (1,), (1,))
    ==> relative_device_ids = [[0,1], [0], [1], [1]])
    """
    # Models and user defined Variables/Tensors would be transferred to the first device.
...
class ReDirectSTD(object):
  ...
  This class overwrites sys.stdout or sys.stderr, so that console logs can also be written to file.
  Args:
    fpath: file path
...
def str2bool(v):
  return v.lower() in ("yes", "true", "t", "1") # whether `v.lower()` in `("yes", "true", "t", "1")`
...
def measure_time(enter_msg):
  st = time.time()
  print(enter_msg)
  yield  # make cpu memory as possibly for other threads in current thread, here will return
  print('Done, {:.2f}s'.format(time.time() - st))  # when caller run over, will return here to print `run time`