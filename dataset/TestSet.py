...
class TestSet(Dataset):
  ...
  def get_sample(self, ptr): # re-product the fun in `Dataset`
  ...
  def next_batch(self): # re-product the fun in `Dataset`
  ...
  def extract_feat(self, normalize_feat):
    ...
    Args:
      normalize_feat: True or False, whether to normalize global feature to unit length
    Returns:
    ...
    """
    global_feats, ids, cams, im_names, marks = \
      [], [], [], [], []
    done = False
    step = 0
    printed = False
    ...
    while not done:
      ...
      global_feat = self.extract_feat_func(ims_)
    ...
    return global_feats, ids, cams, im_names, marks
  ...
  def eval(
    ...
    with measure_time('Computing global distance...'):
      ...
      global_q_g_dist = compute_dist(  # would not use parallel method
    ...
    if to_re_rank:
      with measure_time('Re-ranking...'):
        ...
        global_q_q_dist = compute_dist( # would not use parallel method
          global_feats[q_inds], global_feats[q_inds], type='euclidean')

        # gallery-gallery distance using global distance
        global_g_g_dist = compute_dist( # would not use parallel method
