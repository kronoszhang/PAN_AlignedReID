...
class TrainSet(Dataset):
  ...
  def get_sample(self, ptr):  # re-product the fun in `Dataset`
    ...
    """
    inds = self.ids_to_im_inds[self.ids[ptr]]  # the images of a id
    if len(inds) < self.ims_per_id: # not have enough images to `prefetch` in `Prefetch.py`
      inds = np.random.choice(inds, self.ims_per_id, replace=True)  # with overlap
    else:
      inds = np.random.choice(inds, self.ims_per_id, replace=False) # without overlap
